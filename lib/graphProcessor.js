...
function findRoot(nodes, childrenInComponent) {
  ...
  // Pure cycles: every node is a child; pick lexicographically smallest node as root.
  return [...nodes].sort()[0];
}
...
export function processGraph(edges) {
  ...
  for (const { parent, child } of validEdgeList) {
    allNodes.add(parent);
    allNodes.add(child);
    // Multi-parent rule: first parent in input order wins.
  ...
  for (const { component } of sortedComponents) {
    ...
    if (cyclic) {
      ...
      // largest_tree_root: max depth, lexicographically smallest root on tie.
      if (
        depth > largest_depth ||
        (depth === largest_depth &&
